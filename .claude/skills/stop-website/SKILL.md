...
Stops, in order: `agent_runner.py` -> `joystick-server.py` (:8090) -> the Isaac
Sim + PX4 instance running **this repo's** `sim/bootstrap.py`. SIGTERM first,
SIGKILL after ~10s.
...
pgrep -af "sim/bootstrap.py|joystick-server.py"                   # expect nothing
```
